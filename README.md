...
**Start demo** opens a native full-screen stage in its own macOS Space and hides the editor. Connect an unlocked, trusted iPhone or iPad by USB; the stage attempts to select the sole screen source, then remembers your choice. The video fits the device frame without a separate QuickTime window. **Reconnect** rebuilds the feed, while unplugging and reconnecting waits for the same device. **End demo** or Escape closes the stage and returns to the editor. The stage prevents idle display/system sleep only while open; it cannot override an explicit lock. This new live path is implemented and packaged, with connected-device acceptance testing still pending. **Open QuickTime** remains a manual fallback.
