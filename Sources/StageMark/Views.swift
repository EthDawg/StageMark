...
struct ControlCenter: View {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        if !app.shortcutFailures.isEmpty {
                            Button { app.selectedTab = "Shortcuts" } label: {
    ...
    private var timerSettings: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            Text("Drag or resize the timer window. Closing it keeps the countdown running in the menu bar.").font(.system(size: 11)).foregroundStyle(.secondary)
        }
    }
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack(spacing: 12) {
                ...
                Text(app.timerFinished ? "Ready when you are." : app.timerRunning ? "TAKE A BREATH" : "PAUSED")
                    .font(.system(size: 9, weight: .medium)).tracking(2.5).foregroundStyle(Color(nsColor: settings.value.timerColor.nsColor).opacity(0.4))
                Spacer(minLength: 8)
                HStack(spacing: 18) {
