...
struct QuickControlsView: View {
    ...
    private var timer: some View {
        VStack(spacing: 15) {
            ...
            Text("Closing the timer window keeps its countdown in the menu bar.")
    ...
    private var general: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Share your entire display during a call so the audience can see your annotations.")
                .font(.system(size: 11)).foregroundStyle(.secondary)
            Text("StageMark \(Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "") · Native to macOS").font(.system(size: 10)).foregroundStyle(.tertiary)
        }
    }
