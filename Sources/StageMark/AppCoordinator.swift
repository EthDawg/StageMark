...
final class AppCoordinator: NSObject, ObservableObject, NSWindowDelegate, NSPopoverDelegate {
    ...
    init(settings: SettingsStore = SettingsStore(), archiveURL: URL? = nil) {
    ...
    func start() {
        ...
        createStatusItem(); rebuildScreens(); settingsChanged()
    ...
    func settingsChanged() {
        if registeredShortcuts != settings.value.shortcuts {
            hotkeys.register(settings.value); registeredShortcuts = settings.value.shortcuts
            shortcutFailures = hotkeys.failures
        }
    ...
    private func refreshWindows() {
        ...
        hotkeys.setEscapeEnabled(isDrawing || !boards.isEmpty)
    }
    ...
    func finishRecording() {
        ...
        recordingAction = nil; hotkeys.register(settings.value); registeredShortcuts = settings.value.shortcuts; shortcutFailures = hotkeys.failures
    }
    ...
    func setLaunchAtLogin(_ enabled: Bool) {
        do {
            ...
            launchAtLogin = SMAppService.mainApp.status == .enabled
            if SMAppService.mainApp.status == .requiresApproval { notice = "Approve StageMark in System Settings → General → Login Items."; SMAppService.openSystemSettingsLoginItems() }
